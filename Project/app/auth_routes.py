...
def login():
    ...
    if request.method == 'POST':
        ...
        if not result.data or result.data[0]['role']!=role:
            flash ('No such Account...', 'error')
            return redirect(request.url)

        if result.data[0]['frozen']:
            flash('Your account is frozen temporarily...', 'error')
            return redirect(request.url)
        ...
        if user and user['password'] == password:
            ...
            if role == 'title_verifier':
                ...
                return redirect(url_for('admin_routes.admin_dashboard', username=identifier))
        else:
