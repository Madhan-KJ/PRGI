...
def check_title_rules(title: str) -> bool:
    ...
    for item in disallowed_prefixes_suffixes:
        if item['type'] == 'prefix':
            if title.lower().strip().startswith(item['affix']):
                return False
        else:
            if title.lower().strip().endswith(item['affix']):
                return False
...
@user_routes.route('/title_applicant_dashboard/<username>/', methods=['post','get'])
def title_applicant_dashboard(username):
    check = supabase.table('users').select('notification').eq('username', username).execute().data
    notifications = supabase.table('notification').select('content', 'timestamp').eq('username', username).order('timestamp', desc=True).execute().data
    if request.method == 'get':
        title = ''
    else:
        title= request.form.get('title')
    return render_template('dashboard1.html', user=username, notifications=notifications, check=check[0], title=title)
...
def title_description(id):
    ...
    return render_template('title_desc.html', desc=record[0])
